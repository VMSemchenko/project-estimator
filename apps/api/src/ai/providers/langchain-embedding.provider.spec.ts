...
describe('LangchainEmbeddingProvider', () => {
  ...
  beforeEach(async () => {
    ...
    provider = module.get<LangchainEmbeddingProvider>(LangchainEmbeddingProvider);
  ...
  describe('initialization', () => {
    ...
    it('should call config service with correct keys', () => {
      ...
      expect(mockConfigService.get).toHaveBeenCalledWith('zhipuai.embeddingModel', { infer: true });
    });
  });
  ...
  describe('embedTexts', () => {
    it('should call embedDocuments on the embeddings instance', async () => {
      ...
      const embeddings = provider.getEmbeddings();
      jest.spyOn(embeddings, 'embedDocuments').mockResolvedValue(expectedEmbeddings);
