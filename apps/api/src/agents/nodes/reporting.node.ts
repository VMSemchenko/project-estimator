...
export class ReportingNode extends BaseAgentNode {
  ...
  async execute(state: EstimationState, config?: NodeConfig): Promise<StateUpdate> {
    ...
    try {
      ...
      this.logger.log(`Generated report with total hours: ${this.formatHours(totalHours)}`);
  ...
  private aggregateByRequirement(state: EstimationState): RequirementSummary[] {
    const requirementMap = new Map<string, { hours: number; count: number; title: string }>();
    ...
    for (const estimate of state.estimates) {
      ...
      const existing = requirementMap.get(reqId) || { hours: 0, count: 0, title: 'Unknown' };
  ...
  private calculateTotalHours(estimates: EstimationState['estimates']): number {
    return Math.round(
      estimates.reduce((sum, e) => sum + e.expectedHours, 0) * 10,
    ) / 10;
  }
  ...
  private async generateMarkdownReport(
    ...
    try {
      ...
      this.logger.warn(`LLM report generation failed, using template: ${error}`);
    }
  ...
  private generateTemplateReport(
    ...
    for (const proc of summary.summaryByProcess) {
      lines.push(`| ${proc.processName} | ${this.formatHours(proc.totalHours)} | ${proc.workCount} |`);
    }

    lines.push('', '## Summary by Requirement', '', '| Requirement | Hours | Works |', '|-------------|-------|-------|');

    for (const req of summary.summaryByRequirement) {
      lines.push(`| ${req.requirementTitle} | ${this.formatHours(req.totalHours)} | ${req.workCount} |`);
    }

    lines.push('', '## Detailed Estimates', '');

    for (const estimate of state.estimates) {
      const requirement = state.requirements.find((r) => r.id === estimate.requirementId);
      const work = state.atomicWorks.find((w) => w.id === estimate.atomicWorkId);

      lines.push(`### ${estimate.requirementId}: ${requirement?.title || 'Unknown'}`);
      ...
      lines.push(`  - **Expected:** ${this.formatHours(estimate.expectedHours)}`);
  ...
  private generateCsvContent(state: EstimationState): string {
    ...
    const rows = state.estimates.map((estimate) => {
      ...
      return [
        ...
        estimate.confidence,
        this.escapeCsvField(estimate.appliedCoefficients.map((c) => c.name).join('; ')),
 ...
 */
export function createReportingNode(dependencies: AgentDependencies): ReportingNode {
...
export async function reportingNode(
  ...
  throw new Error('ReportingNode must be created via factory with dependencies');
}
