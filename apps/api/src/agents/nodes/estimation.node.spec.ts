...
describe("EstimationNode", () => {
  ...
  beforeEach(async () => {
    ...
    // Assign mocks to variables for use in tests
    promptsService = mockPromptsService;
    llmProvider = mockLLMProvider;
    langfuseService = mockLangfuseService;
    ragService = mockRagService;
  ...
  describe("execute", () => {
    ...
    it("should handle LLM failure with fallback calculation", async () => {
      ...
      expect(result.estimates?.[0].assumptions).toContain(
        "Fallback estimate due to LLM error",
      );
    });
