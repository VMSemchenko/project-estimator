...
export class DecompositionNode extends BaseAgentNode {
  ...
  private readonly catalogsService: CatalogsService | null;

  constructor(dependencies: AgentDependencies) {
    super(dependencies);
    this.ragService = dependencies.ragService || null;
    this.catalogsService = dependencies.catalogsService || null;
  }
  ...
  async execute(
    ...
    try {
      ...
      this.logger.log(
        `Created ${atomicWorksMappings.length} atomic work mappings`,
      );
  ...
  private async mapRequirementToAtomicWorks(
    ...
    // Get BA processes catalog from catalogs service
    const baProcesses = this.catalogsService?.getAllBaProcesses() || [];
    ...
    const context: PromptContext = {
      ...
      baProcessesCatalog: baProcesses.map((process) => ({
        id: process.id,
        name: process.name,
    ...
    try {
      ...
      // Ensure mappings array exists and is valid
      const mappings = response?.mappings;
      if (!mappings || !Array.isArray(mappings)) {
        this.logger.warn(
          "LLM response missing mappings array, using fallback from RAG results",
        );
   ...
   */
  groupByProcess(
    mappings: AtomicWorkMapping[],
  ): Map<string, AtomicWorkMapping[]> {
 ...
 */
export function createDecompositionNode(
...
export async function decompositionNode(
  ...
  throw new Error(
    "DecompositionNode must be created via factory with dependencies",
  );
}
